int sensorPin = A1; //Pin to connect the sensor
...
void setup() {
pinMode(buzzerPin, OUTPUT); //We set the pin that we connected the buzzer to OUTPUT.
Serial.begin(9600);
}
void loop() {
...
delay(100);
} else { //If the sensor data is less than the threshold value, the codes in the if are applied.
digitalWrite(buzzerPin, LOW);
}
}